...
export default function UrbanDepartments({ lang, onOpenApplyModal }) {
  ...
  return (
    <section id="departments" className="py-14 bg-white border-b border-slate-200 font-sans">
      <div className="max-w-7xl mx-auto px-4">
        {/* Section Header with Mehsana Urban style maroon accent */}
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8 border-b-2 border-slate-200 pb-4">
          <div>
            <div className="text-xs font-extrabold uppercase tracking-wider text-[#800000] mb-1">
              {lang === "gu" ? "શૈક્ષણિક કાર્યક્રમો અને વિભાગો" : "Academic Faculties & Departments"}
            </div>
            <h2 className="text-2xl sm:text-3xl font-black text-[#1b3557] tracking-tight">
              {lang === "gu" ? "અભ્યાસક્રમો અને શૈક્ષણિક વિગતો (NEP 2020)" : "Programs of Study & NEP 2020 Structure"}
            </h2>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredPrograms.map((course) => (
            <div
              key={course.id}
              className="bg-white rounded-xl border border-slate-200 hover:border-[#1b3557] shadow-sm hover:shadow-md transition-all flex flex-col justify-between overflow-hidden group"
            >
              {/* Card Header Strip */}
              <div className="bg-[#1b3557] group-hover:bg-[#800000] transition-colors text-white p-4 flex items-center justify-between">
                <div>
                ...
                <div>
                  <div className="space-y-2 text-xs bg-slate-50 p-3 rounded-lg border border-slate-100 mb-4">
                    <div className="flex items-center justify-between">
                      <span className="text-slate-500 font-semibold">{lang === "gu" ? "અવધિ:" : "Duration:"}</span>
                      <span className="font-bold text-slate-800">{course.duration}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-slate-500 font-semibold">{lang === "gu" ? "પ્રવેશ ક્ષમતા:" : "Intake Capacity:"}</span>
                      <span className="font-bold text-[#800000]">{course.intake} Seats</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-slate-500 font-semibold">{lang === "gu" ? "શિક્ષણ માધ્યમ:" : "Medium:"}</span>
                      <span className="font-bold text-slate-800">{course.medium}</span>
                    </div>
                    <div className="pt-1.5 border-t border-slate-200 text-slate-600">
                      <span className="font-semibold text-slate-500 block mb-0.5">{lang === "gu" ? "પ્રવેશ પાત્રતા:" : "Eligibility:"}</span>
                  ...
                  <div className="mb-4">
                    <div className="text-[11px] font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                      {lang === "gu" ? "મુખ્ય વિષયો / પેપર્સ:" : "Core Subjects & Specializations:"}
                    </div>
        ...
        {selectedCourseForModal && (
          <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 animate-fadeIn">
            <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 shadow-2xl border border-slate-200">
              <div className="flex items-start justify-between gap-4 border-b-2 border-[#800000] pb-3 mb-4">
                <div>
                  ...
                  <h3 className="text-xl font-black text-[#1b3557] mt-0.5">
              ...
              <div className="mb-4">
                <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">
                  {lang === "gu" ? "સેમેસ્ટર વાઇઝ વિષયોની સૂચિ:" : "Full Subjects & Curriculum Topics:"}
                </h4>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {selectedCourseForModal.subjects.map((sub, idx) => (
                    <div key={idx} className="p-2.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-semibold text-slate-800 flex items-center gap-2">
                      <span className="w-5 h-5 rounded-full bg-[#1b3557] text-white flex items-center justify-center text-[10px] font-bold shrink-0">
                        {idx + 1}
                      </span>
                      <span>{sub}</span>
                    </div>
